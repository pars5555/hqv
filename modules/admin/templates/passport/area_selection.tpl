<div class="col s12 m2 4">
    ...
    <select id="p_area" class="browser-default">
        {foreach from=$ns.all_areas item=area}
            <option value="{$area->getId()}" data-region="{$area->getRegion()}" data-community="{$area->getCommunity()}"
                    {if $ns.selectedAreaId == $area->getId()}selected{/if}>{$area->getTerritoryId()}/{$area->getAreaId()}</option>
        {/foreach}
    </select>
</div>    
<div class="col s12 m2 4">
