...
namespace ngsadmin\dal\mappers {
  ...
  class AdminMapper extends \ngs\framework\dal\mappers\AbstractMysqlMapper {
    ...
    private $GET_ADMIN_BY_USERNAME_AND_PASS = "SELECT * from `%s` WHERE `username` = :username AND `password` = :password";

    public function getByUsernamePassword($username, $password) {
      $sqlQuery = sprintf($this->GET_ADMIN_BY_USERNAME_AND_PASS, $this->getTableName());
